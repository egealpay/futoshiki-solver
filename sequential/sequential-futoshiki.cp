    ...
    while (!file.eof()) {
        ...
        if (constraint != "") {
            ...
            constraints.push_back(
                    std::pair<std::pair<int, int>, std::pair<int, int> >(std::pair<int, int>(val1 - 1, val2 - 1),
...
bool isSatisfyConstraints(int **&matrix, int currentRow, int currentCol, int value,
    ...
    for (int i = 0; i < numConstraints; i++) {
        ...
        if (currentRow == start.first && currentCol == start.second) {
            if (value < matrix[end.first][end.second] && matrix[end.first][end.second] != -1)
                return false;
        } else if (currentRow == end.first && currentCol == end.second) {
...
bool doesSatisfyRules(int **&matrix, int currentRow, int currentCol, int value,
                      std::vector<std::pair<std::pair<int, int>, std::pair<int, int> > > &constraints, int size) {
    return canUseInRow(matrix, currentRow, value, size) &&
...
bool solve(int **&matrix, std::vector<std::pair<std::pair<int, int>, std::pair<int, int> > > &constraints, int size) {
    // Find a empty cell
    ...
    for (int value = 1; value <= size; value++) {
        if (doesSatisfyRules(matrix, currentRow, currentCol, value, constraints, size)) {
            ...
            if (solve(matrix, constraints, size))
                return true;
